...
if(isset($_SESSION['dataform1']) ){
   //echo "<pre>";
	...
	if((isset($enrollment_min) && !empty($enrollment_min))  || (isset($enrollment_max) && !empty($enrollment_max)) ){
		if(empty($enrollment_min) && !empty($enrollment_max)):
			if($enrollment_max == '40000+'):
				$enrollment_max = "(SELECT MAX(ENRTOT) FROM `drvef2019`)";
			endif;
			$qry = "SELECT unitid FROM `drvef2019` WHERE `ENRTOT` <= ".$enrollment_max." ";
		elseif(empty($enrollment_max) && !empty($enrollment_min)):
			if($enrollment_max == '40000+'):
				$enrollment_max = "(SELECT MAX(ENRTOT) FROM `drvef2019`)";
			endif;
			$qry = "SELECT unitid FROM `drvef2019` WHERE `ENRTOT` >= ".$enrollment_min." ";
		elseif(!empty($enrollment_max) && !empty($enrollment_min)):
			if($enrollment_max == '40000+'):
				$enrollment_max = "(SELECT MAX(ENRTOT) FROM `drvef2019`)";
			endif;
			$qry = "SELECT unitid FROM `drvef2019` WHERE `ENRTOT` >= ".$enrollment_min." AND `ENRTOT` <= ".$enrollment_max." ";
		endif;
	...
	if(isset($level_of_award) && !empty($level_of_award)){		
		...
		$condition .= " AND `UNITID` IN( SELECT DISTINCT(`UNITID`) FROM `c2019_a` WHERE `AWLEVEL` IN (".$awards_key.") AND MAJORNUM = 1 )";
	}
	...
	if(isset($program_majors)){
		...
		$condition .= " AND `UNITID` IN(SELECT `UNITID` FROM `c2019dep` WHERE ".$programsCat_data." )";		
	}
...
*/
	...
	if(isset($housing) && !empty($housing)){		
		$condition .= " AND `UNITID` IN( SELECT DISTINCT(`UNITID`) FROM `ic2019` WHERE `ROOM` = 1 )";
	}
	...
	if(isset($extended_learning) && !empty($extended_learning)){
		...
		$condition .= " AND (`UNITID` IN( SELECT DISTINCT(`UNITID`) FROM `ic2019` WHERE ".$extend_search."))";
	...
	if(!empty($ReligiousAffilation)){
		$condition .= " AND `UNITID` IN(SELECT `UNITID` FROM `ic2019` WHERE `RELAFFIL` = '".$ReligiousAffilation."' )";
	}
	...
	if(isset($_SESSION['dataform1']['feestype']))
	{
		$feestype = $_SESSION['dataform1']['feestype'];
	}	
	...
	*/
	// Tuition & Fees 
	if(!empty($feestype) &&  (!empty($tuitionMax) || !empty($tuitionMin))){
		$tuitionids = array();
		if($feestype=='averagenetprice'){
			$column = 'NPIST2';
			if (!empty($tuitionMin) && !empty($tuitionMax)) {

				$ids_sql = "SELECT UNITID FROM `sfa1819_p2` WHERE `" . $column . "` >= '" . $tuitionMin . "' AND `" . $column . "` <= '" . $tuitionMax . "'";
				//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."' AND `CHG2AY3` <= '".$tuitionMax."'";

			} elseif (!empty($tuitionMin) && empty($tuitionMax)) {
				$ids_sql = "SELECT UNITID FROM `sfa1819_p2` WHERE `" . $column . "` >= '" . $tuitionMin . "'";
				//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."'";

			} elseif (empty($tuitionMin) && !empty($tuitionMax)) {
				$ids_sql = "SELECT UNITID FROM `sfa1819_p2` WHERE `" . $column . "` <= '" . $tuitionMax . "'";
				//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` <= '".$tuitionMax."'";
			}
		...
		}else if($feestype != 'netprice') {
			if ($feestype == 'instate') {
				$column = 'CHG2AY3';
			} else if ($feestype == 'outofstate') {
				$column = 'CHG3AY3';
			...
			if (!empty($tuitionMin) && !empty($tuitionMax)) {
				...
				$ids_sql = "SELECT UNITID FROM `ic2019_ay` WHERE `" . $column . "` >= '" . $tuitionMin . "' AND `" . $column . "` <= '" . $tuitionMax . "'";
				//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."' AND `CHG2AY3` <= '".$tuitionMax."'";

			} elseif (!empty($tuitionMin) && empty($tuitionMax)) {
				$ids_sql = "SELECT UNITID FROM `ic2019_ay` WHERE `" . $column . "` >= '" . $tuitionMin . "'";
				//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."'";

			} elseif (empty($tuitionMin) && !empty($tuitionMax)) {
				$ids_sql = "SELECT UNITID FROM `ic2019_ay` WHERE `" . $column . "` <= '" . $tuitionMax . "'";
				//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` <= '".$tuitionMax."'";
			}
	...
	if(!empty($applicantsAcceptedMin) || !empty($applicantsAcceptedMax)){		
		$ids = array();
		if(!empty($applicantsAcceptedMin) && !empty($applicantsAcceptedMax) ){	
			$ids_sql = "SELECT UNITID FROM `drvadm2019` WHERE `DVADM01` >= '".$applicantsAcceptedMin."' AND `DVADM01` <= '".$applicantsAcceptedMax."'";
			

		}elseif (!empty($applicantsAcceptedMin) && empty($applicantsAcceptedMax)) {
			$ids_sql = "SELECT UNITID FROM `drvadm2019` WHERE `DVADM01` >= '".$applicantsAcceptedMin."'";
			
			
		}elseif (empty($applicantsAcceptedMin) && !empty($applicantsAcceptedMax)) {
			$ids_sql = "SELECT UNITID FROM `drvadm2019` WHERE `DVADM01` <= '".$applicantsAcceptedMax."' AND `DVADM01` <> 0 ";
		}
		//echo $ids_sql;
		$ids_result = $con->query($ids_sql);
		if ($ids_result->num_rows > 0) {
	...
	if(!empty($graduationrateMin) || !empty($graduationrateMax)){
		$ids = array();
		if(!empty($graduationrateMin) && !empty($graduationrateMax) ){
			$ids_sql = "SELECT `PGGRRTT`,`UNITID` FROM `drvgr2019` WHERE  `drvgr2019`.`PGGRRTT` >= '".$graduationrateMin."' AND `drvgr2019`.`PGGRRTT` <= '".$graduationrateMax."'";


		}elseif (!empty($graduationrateMin) && empty($graduationrateMax)) {
			$ids_sql = "SELECT `PGGRRTT`,`UNITID` FROM `drvgr2019` WHERE  `drvgr2019`.`PGGRRTT` >= '".$graduationrateMin."' ";




		}elseif (empty($graduationrateMin) && !empty($graduationrateMax)) {
			$ids_sql = "SELECT `PGGRRTT`,`UNITID` FROM `drvgr2019` WHERE  `drvgr2019`.`PGGRRTT` <= '".$graduationrateMax."'";
	...
	if(!empty($financialaidMin) || !empty($financialaidMax)){
		$ids = array();
		if(!empty($financialaidMin) && !empty($financialaidMax) ){
			$ids_sql = "SELECT * FROM `sfa1819_p1` WHERE  `sfa1819_p1`.`ANYAIDP` >= '".$financialaidMin."' AND `sfa1819_p1`.`ANYAIDP` <= '".$financialaidMax."'";


		}elseif (!empty($financialaidMin) && empty($financialaidMax)) {
			$ids_sql = "SELECT * FROM `sfa1819_p1` WHERE  `sfa1819_p1`.`ANYAIDP` >= '".$financialaidMin."' ";




		}elseif (empty($financialaidMin) && !empty($financialaidMax)) {
			$ids_sql = "SELECT * FROM `sfa1819_p1` WHERE  `sfa1819_p1`.`ANYAIDP` <= '".$financialaidMax."'";
	...
	if(isset($_SESSION['dataform1']['satevidenceMax']))
	{
		...
		$tuitionids = array();
			...
			if (!empty($satevidenceMin) && !empty($satevidenceMax)) {
				...
				$ids_sql = "SELECT UNITID FROM `adm2019` WHERE `" . $column . "` >= '" . $satevidenceMin . "' AND `" . $column . "` <= '" . $satevidenceMax . "'";
				//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."' AND `CHG2AY3` <= '".$tuitionMax."'";

			} elseif (!empty($satevidenceMin) && empty($satevidenceMax)) {
				$ids_sql = "SELECT UNITID FROM `adm2019` WHERE `" . $column . "` >= '" . $satevidenceMin . "'";
				//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."'";

			} elseif (empty($satevidenceMin) && !empty($satevidenceMax)) {
				$ids_sql = "SELECT UNITID FROM `adm2019` WHERE `" . $column . "` <= '" . $satevidenceMax . "'";
				//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` <= '".$tuitionMax."'";
			}
	...
	if(isset($_SESSION['dataform1']['satmathMax']))
	{
		...
		if (!empty($satmathMin) && !empty($satmathMax)) {
			...
			$ids_sql = "SELECT UNITID FROM `adm2019` WHERE `" . $column . "` >= '" . $satmathMin . "' AND `" . $column . "` <= '" . $satmathMax . "'";
			//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."' AND `CHG2AY3` <= '".$tuitionMax."'";

		} elseif (!empty($satmathMin) && empty($satmathMax)) {
			$ids_sql = "SELECT UNITID FROM `adm2019` WHERE `" . $column . "` >= '" . $satmathMin . "'";
			//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."'";

		} elseif (empty($satmathMin) && !empty($satmathMax)) {
			$ids_sql = "SELECT UNITID FROM `adm2019` WHERE `" . $column . "` <= '" . $satmathMax . "'";
			//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` <= '".$tuitionMax."'";
		}
	...
	if(isset($_SESSION['dataform1']['actengMax']))
	{
		...
		if (!empty($actengMin) && !empty($actengMax)) {
			...
			$ids_sql = "SELECT UNITID FROM `adm2019` WHERE `" . $column . "` >= '" . $actengMin . "' AND `" . $column . "` <= '" . $actengMax . "'";
			//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."' AND `CHG2AY3` <= '".$tuitionMax."'";

		} elseif (!empty($satmathMin) && empty($satmathMax)) {
			$ids_sql = "SELECT UNITID FROM `adm2019` WHERE `" . $column . "` >= '" . $actengMin . "'";
			//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."'";

		} elseif (empty($satmathMin) && !empty($satmathMax)) {
			$ids_sql = "SELECT UNITID FROM `adm2019` WHERE `" . $column . "` <= '" . $actengMax . "'";
			//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` <= '".$tuitionMax."'";
		}
      	$ids_result = $con->query($ids_sql);
		if ($ids_result->num_rows > 0) {
	...
	if(isset($_SESSION['dataform1']['actmathMax']))
	{
		...
		if (!empty($actmathMin) && !empty($actmathMax)) {
			...
			$ids_sql = "SELECT UNITID FROM `adm2019` WHERE `" . $column . "` >= '" . $actmathMin . "' AND `" . $column . "` <= '" . $actmathMax . "'";
			//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."' AND `CHG2AY3` <= '".$tuitionMax."'";

		} elseif (!empty($actmathMin) && empty($actmathMax)) {
			$ids_sql = "SELECT UNITID FROM `adm2019` WHERE `" . $column . "` >= '" . $actmathMin . "'";
			//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."'";

		} elseif (empty($actmathMin) && !empty($actmathMax)) {
			$ids_sql = "SELECT UNITID FROM `adm2019` WHERE `" . $column . "` <= '" . $actmathMax . "'";
			//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` <= '".$tuitionMax."'";
		}
		$ids_result = $con->query($ids_sql);
		if ($ids_result->num_rows > 0) {
	...
	<div class="container-fluid">
		<div class="row">
			<div class="col-sm-12 col-md-12 col-lg-12 col-xl-12 px-0 overflow-hide">
				<div id="progress-bar">
					<div id="1" class="done one progress-bar-item progress-bar-item-edges" ><a href="guest_form.php" class="white-font"><p class="progress-bar-text">My Filters</p></a></div>
					
					<div id="2"class="done two progress-bar-item progress-bar-item-edges" ><a href="guest_college-search.php" class="white-font"><p class="progress-bar-text">Results</p></a></div>
					
					<div id="3"class=" results progress-bar-item progress-bar-item-edges" ><a href="#!" class="white-font"><p class="progress-bar-text">My Profile</p></a></div>
...
<ul class="list-group" style="margin-bottom: 0px;"> 
  ...
  <li class="list-group-item list-group-results colg-advanced-search-btn adv-search-res ad-srch" >Advanced Search<i class="arrow_icon fa fa-caret-right" aria-hidden="true"></i></li>
	<ul class="list-group advanced-search-output advance-search-menu">
		...
		<li class="list-group-item list-group-results inst-type" data-toggle="modal" data-target="#diversityAdvance">Diversity<i class="arrow_icon fa fa-caret-right" aria-hidden="true"></i></li>
  ...
  <!--<input type="submit" value="Apply Filters" class="btn btn-primary apj-chance-of-acceptance mybluebtn" style="margin-bottom: 32px;background: #019ff0;color: #fff;padding: 16px 62px;outline: none;display: none; margin-left:auto;margin-right:auto;width: 100% !important;" id="applyfilters" name="applyfilters">-->
   <!--  <input type="submit" value="Chance of Acceptance?" class="btn btn-primary apj-chance-of-acceptance" style="margin-bottom: 32px;background: #019ff0;color: #fff;padding: 20px 62px;outline: none;"> --> 
	...
	<div id="satActScore" class="advance-filter-modal modal fade" role="dialog">
	    <div class="modal-dialog modal-lg">
	        <div class="modal-content">
	            ...
	            <?php if($row['credits'] > 4) {?>
	            <!-- <div class="modal-footer">
					<input type="submit" value="Apply" class="btn btn-primary apj-chance-of-acceptance actsatbtn mybluebtn" id="applyfilters" name="applyfilters">
	            </div> -->
	            <?php } else{?>
	                <!-- <div class="modal-footer pb-15px">
					<input type="submit" value="Apply" class="btn btn-primary apj-chance-of-acceptance mybluebtn" id="applyfilters" name="applyfilters" disabled>
	    ...
	    <div class="modal-dialog modal-lg">
	        <div class="modal-content">
				...
				<div class="mform_field_wrap">
						<label class="mod_style_outer mb-0 radio-butn">
						<div class="radio custom-radio-style col-md-4 apj-radio" style="margin:0">
								<label><input type="radio" style="position:inherit;" name="feestype" id="outofstate" value="outofstate"> Out-of-State Tuition</label>
							</div>
							<div class="radio custom-radio-style col-md-4 apj-radio" style="margin:0">
								<label><input type="radio" style="position:inherit;" name="feestype" id="instate" value="instate"> In-State Tuition</label>
							</div>
							<div class="radio custom-radio-style col-md-4 apj-radio" style="margin:0">
								<label><input type="radio" style="position:inherit;" name="feestype" id="books" value="books"> Books & Supplies</label>
							</div>
							<div class="radio custom-radio-style col-md-4 apj-radio" style="margin:0">
								<label><input type="radio" style="position:inherit;" name="feestype" id="netprice" value="netprice"> Net Price</label>
							</div>
							
							<div class="radio custom-radio-style col-md-4 apj-radio" style="margin:0">
								<label><input type="radio" style="position:inherit;" name="feestype" id="averagenetprice" value="averagenetprice"> Average Net Price</label>
							</div>
						</label>
						</div>
				   <!-- radio -->
	...
	<!--Diversity-->
	<div id="diversityAdvance" class="advance-filter-modal modal fade" role="dialog">
	    <div class="modal-dialog modal-lg">
	        <div class="modal-content">
	            <div class="modal-header">
	              <button type="button" class="close" data-dismiss="modal">&times;</button>
	              <h4 class="modal-title">Diversity</h4>
	            </div>
	            <div class="modal-body">
				...
				<div class="mform_field_wrap">
						<label class="mod_style_outer mb-0 radio-butn">
						<div class="radio custom-radio-style apj-radio" style="margin:0">
								<label><input type="radio" style="position:inherit;" name="diversityAdvanceRadio" id="americanIndian" value="American indian or Alaska Native"> American indian or Alaska Native</label>
							</div>
							<div class="radio custom-radio-style apj-radio" style="margin:0">
								<label><input type="radio" style="position:inherit;" name="diversityAdvanceRadio" id="asianPacific" value="Asian/Native Hawalian/Pacific Islander"> Asian/Native Hawalian/Pacific Islander</label>
							</div>
							<div class="radio custom-radio-style apj-radio" style="margin:0">
								<label><input type="radio" style="position:inherit;" name="diversityAdvanceRadio" id="blackAfAm" value="Black or African American"> Black or African American</label>
							</div>
							<div class="radio custom-radio-style apj-radio" style="margin:0">
								<label><input type="radio" style="position:inherit;" name="diversityAdvanceRadio" id="hisLatino" value="Hispanic / Latino"> Hispanic / Latino</label>
							</div>
							<div class="radio custom-radio-style apj-radio" style="margin:0">
								<label><input type="radio" style="position:inherit;" name="diversityAdvanceRadio" id="whiteDiv" value="White"> White</label>
							</div>
						</label>
						</div>
				   <!-- radio -->
	                ...
	                <div class="mform_field_wrap" id="courseoptions">
	                    <label class="mod_style_outer mb-0" style="width:100%;line-height: 35px;">
	                      <select class="js-example-basic-multiple form-control" id="program_majors" name="program_majors[]" multiple="multiple">  
	                              <?php				  			
	                                $queryvertical = "SELECT * FROM `valuesets19` WHERE `TableName` = 'C2019DEP' AND `varName` = 'CIPCODE'  ORDER BY `valuesets19`.`valueLabel` ASC ";					
...
<div class="col-md-9">
	...
	<?php //print_r($_SESSION['clgcb']); ?>
				...
				<div class="row p15">
					...
					<div class="col-md-4 col-xs-12 text-right apj-top-select mb-0-desktop">
    					<select  class="form-control apj-top-selectbox" name="orderbyfield" id="orderbyfield">
							...
							<option value="outofstate"  <?php if(isset($_SESSION['orderbyfield']) && !empty($_SESSION['orderbyfield'])){ if($_SESSION['orderbyfield'] == 'outofstate'){ echo "selected"; }} ?>>Out-of-State Tuition</option>
							<option value="instate"  <?php if(isset($_SESSION['orderbyfield']) && !empty($_SESSION['orderbyfield'])){ if($_SESSION['orderbyfield'] == 'instate'){ echo "selected"; }} ?>>In-State Tuition</option>
							<option value="books"  <?php if(isset($_SESSION['orderbyfield']) && !empty($_SESSION['orderbyfield'])){ if($_SESSION['orderbyfield'] == 'books'){ echo "selected"; }} ?>>Books & Supplies</option>
							<option value="netprice"  <?php if(isset($_SESSION['orderbyfield']) && !empty($_SESSION['orderbyfield'])){ if($_SESSION['orderbyfield'] == 'netprice'){ echo "selected"; }} ?>>Net Price</option>
							<option value="averagenetprice"  <?php if(isset($_SESSION['orderbyfield']) && !empty($_SESSION['orderbyfield'])){ if($_SESSION['orderbyfield'] == 'averagenetprice'){ echo "selected"; }} ?>>Average Net Price</option>
    					</select>
					</div>
					<div class="col-md-4 col-xs-12 text-right apj-top-select mb-0-desktop">
		...
		<?php
			$sql = "SELECT count(*) as totalcount FROM hd2019 ".$condition ;
			$result = $con->query($sql);	
			echo '<div class="col-md-3 col-xs-5 text-right result-number-xs">';
			while($row = $result->fetch_assoc()) {
			echo '<span class="search_results">'.$row['totalcount'].' results</span><input type="hidden" id="selectall_clg_count" name="selectall_clg_count" value="'.$row['totalcount'].'">';
			}
			echo '</div>';
		?>
		<!--search result number-->
	...
	<div class="row">
		...
		<div class="myresultalert select-all-btn">
			<div class="col-sm-8 col-md-8 col-xs-12 selectcheckbox">
			<div class="checkbox">
  				<label><input type="checkbox" id="chk_selectallclg"  name="chk_selectallclg" onclick="selectallclgs()"><p id="txt_selectallclg">Select All Colleges</p></label>
			</div>
			</div>
			<div class="col-sm-4 col-md-4 col-xs-12 selectbutton">
				<div class="view-college-btn">
					<a href=# class="selected_clgs_popup">View Selected Colleges</a>
				</div>
			</div>
		</div>
	</div>
	...
	<!-- alert -->
	<!-- Modal -->
	<div class="modal fade" id="selectallcollegemodal" role="dialog">
	    <div class="modal-dialog">
	      ...
	      <div class="modal-content">
	        <div class="modal-header">
	          <button type="button" class="close" data-dismiss="modal">&times;</button>
	          <h4 class="modal-title">Select All Colleges</h4>
	        </div>
	        <div class="modal-body">
	          <p>You have exceeded the maximum number of schools that can be selected at once using the Select All Colleges feature. This features allows you to select up to 250 colleges at a time. You may update your filters to narrow your search results or select colleges individually.</p>
	        </div>
	      </div>
	...
	<form action="guest_form2.php" method="post">
		...
		if(isset($_SESSION['dataform1']) ):
				...
				$sql = "SELECT * FROM hd2019 ".$condition;
				...
				if(isset($_SESSION['orderbyfield']) && isset($_SESSION['orderbytype']) && !empty($_SESSION['orderbytype']) && !empty($_SESSION['orderbyfield']))
				{
					...
					{
						$sql .= ' ORDER BY '."(select `ENRTOT` from `drvef2019` where drvef2019.UNITID = hd2019.UNITID) ".$_SESSION['orderbytype'];
					}	
					else if($_SESSION['orderbyfield'] == 'percentadmitted')
					{
						$sql .= ' ORDER BY '."(select `DVADM01` from `drvadm2019` where drvadm2019.UNITID = hd2019.UNITID) ".$_SESSION['orderbytype'];
					}	
					else if($_SESSION['orderbyfield'] == 'graduationrate')
					{
						$sql .= ' ORDER BY '."(select `PGGRRTT` from `drvgr2019` where drvgr2019.UNITID = hd2019.UNITID) ".$_SESSION['orderbytype'].' ';
					}
					else if($_SESSION['orderbyfield'] == 'finacialaid')
					{
						$sql .= ' ORDER BY '."(select `ANYAIDP` from `sfa1819_p1` where sfa1819_p1.UNITID = hd2019.UNITID) ".$_SESSION['orderbytype'].' ';
					}
					else if($_SESSION['orderbyfield'] == 'outofstate')
					{				

						$sql .= ' ORDER BY '."(SELECT `CHG3AY3` FROM `ic2019_ay` where ic2019_ay.UNITID = hd2019.UNITID) ".$_SESSION['orderbytype'].' ';
					}else if($_SESSION['orderbyfield'] == 'instate')
					{				

						$sql .= ' ORDER BY '."(SELECT `CHG2AY3` FROM `ic2019_ay` where ic2019_ay.UNITID = hd2019.UNITID) ".$_SESSION['orderbytype'].' ';
					}else if($_SESSION['orderbyfield'] == 'books')
					{
						///$ids_sql = "SELECT UNITID FROM `ic2018_py` WHERE `CHG1PY3` >= '".$tuitionMin."'";
						//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."'";

						$sql .= ' ORDER BY '."(SELECT `CHG4AY3` FROM `ic2019_ay` where ic2019_ay.UNITID = hd2019.UNITID) ".$_SESSION['orderbytype'].' ';
					}else if($_SESSION['orderbyfield'] == 'netprice')
					{
						///$ids_sql = "SELECT UNITID FROM `ic2018_py` WHERE `CHG1PY3` >= '".$tuitionMin."'";
						//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."'";

						$sql .= ' ORDER BY '."(SELECT `NPIST2` FROM `sfa1819_p2` where sfa1819_p2.UNITID = hd2019.UNITID) ".$_SESSION['orderbytype'].' ';
					}else if($_SESSION['orderbyfield'] == 'averagenetprice')
					{
						///$ids_sql = "SELECT UNITID FROM `ic2018_py` WHERE `CHG1PY3` >= '".$tuitionMin."'";
						//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."'";
					...
					}else if($_SESSION['orderbyfield'] == 'satevidence')
					{
						...
						$sql .= ' ORDER BY '."(SELECT `SATVR25` FROM `adm2019` where adm2019.UNITID = hd2019.UNITID) ".$_SESSION['orderbytype'].' ';
					}else if($_SESSION['orderbyfield'] == 'satmath') {
						///$ids_sql = "SELECT UNITID FROM `ic2018_py` WHERE `CHG1PY3` >= '".$tuitionMin."'";
						//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."'";

						$sql .= ' ORDER BY ' . "(SELECT `SATMT25` FROM `adm2019` where adm2019.UNITID = hd2019.UNITID) " . $_SESSION['orderbytype'] . ' ';
					}else if($_SESSION['orderbyfield'] == 'actenglish')
					{
						///$ids_sql = "SELECT UNITID FROM `ic2018_py` WHERE `CHG1PY3` >= '".$tuitionMin."'";
						//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."'";

						$sql .= ' ORDER BY '."(SELECT `ACTEN25` FROM `adm2019` where adm2019.UNITID = hd2019.UNITID) ".$_SESSION['orderbytype'].' ';
					}else if($_SESSION['orderbyfield'] == 'actmath')
					{
						///$ids_sql = "SELECT UNITID FROM `ic2018_py` WHERE `CHG1PY3` >= '".$tuitionMin."'";
						//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."'";

						$sql .= ' ORDER BY '."(SELECT `ACTMT25` FROM `adm2019` where adm2019.UNITID = hd2019.UNITID) ".$_SESSION['orderbytype'].' ';
					}
					else
					{
				...
				//	echo '<input type="submit" name="submit" value="Analyze Colleges !">';
					...
					if(count($clgidarr)>0){
						...
						foreach($clgidarr as $arr){
							...
							{
								$enrollquery = "select `ENRTOT` from `drvef2019` where drvef2019.UNITID =".$arr['UNITID'];
								...
								$percentquery = "select `DVADM01` from `drvadm2019` where drvadm2019.UNITID =".$arr['UNITID'];
								...
								$percentquery = "select `PGGRRTT` from `drvgr2019` where drvgr2019.UNITID =".$arr['UNITID'];
								$percentresult = $con->query($percentquery);
								$rowpercent = $percentresult->fetch_assoc();
								if(!empty($rowpercent)){
								...
								$percentquery = "select `ANYAIDP` from `sfa1819_p1` where sfa1819_p1.UNITID =".$arr['UNITID'];
								$percentresult = $con->query($percentquery);
								$rowpercent = $percentresult->fetch_assoc();
								if(!empty($rowpercent)){
								...
								$percentquery = "SELECT `CHG3AY3` FROM `ic2019_ay` where ic2019_ay.UNITID =".$arr['UNITID'];
								$percentresult = $con->query($percentquery);
								$rowpercent = $percentresult->fetch_assoc();
								if(!empty($rowpercent)){
								...
								$percentquery = "SELECT `CHG2AY3` FROM `ic2019_ay` where ic2019_ay.UNITID =".$arr['UNITID'];
								$percentresult = $con->query($percentquery);
								$rowpercent = $percentresult->fetch_assoc();
								if(!empty($rowpercent)){
								...
								$percentquery = "SELECT `CHG4AY3` FROM `ic2019_ay` where ic2019_ay.UNITID =".$arr['UNITID'];
								$percentresult = $con->query($percentquery);
								$rowpercent = $percentresult->fetch_assoc();
								if(!empty($rowpercent)){
								...
								$percentquery = "SELECT `NPIST2` FROM `sfa1819_p2` where sfa1819_p2.UNITID =".$arr['UNITID'];
								$percentresult = $con->query($percentquery);
								$rowpercent = $percentresult->fetch_assoc();
								if(!empty($rowpercent)){
								...
								$percentquery = "SELECT `SATVR25` FROM `adm2019` where adm2019.UNITID =".$arr['UNITID'];
								$percentresult = $con->query($percentquery);
								$rowpercent = $percentresult->fetch_assoc();
								if(!empty($rowpercent)){
								...
								$percentquery = "SELECT `SATMT25` FROM `adm2019` where adm2019.UNITID =".$arr['UNITID'];
								$percentresult = $con->query($percentquery);
								$rowpercent = $percentresult->fetch_assoc();
								if(!empty($rowpercent)){
								...
								$percentquery = "SELECT `ACTEN25` FROM `adm2019` where adm2019.UNITID =".$arr['UNITID'];
								$percentresult = $con->query($percentquery);
								$rowpercent = $percentresult->fetch_assoc();
								if(!empty($rowpercent)){
								...
								$percentquery = "SELECT `ACTMT25` FROM `adm2019` where adm2019.UNITID =".$arr['UNITID'];
								$percentresult = $con->query($percentquery);
								$rowpercent = $percentresult->fetch_assoc();
								if(!empty($rowpercent)){
								 ...
								 <div class="College-Search-inner" id="box'.$arr['UNITID'].'" '.$kala_backgroundclr.'>
								   <div class="col-sm-12 col-md-11 col-xs-12 pr-0" onclick="toggleClgUpper('.$arr['UNITID'].')">
										...
										if(isset($_SESSION['orderbyfield']) && $_SESSION['orderbyfield'] == 'netprice') {
											echo '<p>Net Price : ' . '$'.number_format($netprice) . '</p>';
										}
										*/
								   ...
								   echo '</div>
									<div class="col-sm-12 col-md-12 col-xs-12 btn-college-list">
									  <div class="collage-details table-responsive-sm">
									   <ul>
											 <li>
												<a class="clg-website-address mybluebtnsmall '.$kala_btnclass.'" href="school-profile/'.$arr['slug'].'" target="_blank">More Information</a>
											</li>
										  </ul>
										   </div> 
									</div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body" id="selectedclg">
       <div class="col-xs-12 col-md-12 mb-30">
        <ul class="list-group mycollegelist 123" id="myselectedclgcb">
					...
					?>
					 <li class="list-group-item">
						...
						<span class="deleteclg">
						 ...
						 <a href="school-profile/'.$row['slug'].'" target="_blank">
...
<script> 
function selectallclgs()
{
	var flag = 0;
	if($('#chk_selectallclg').is(':checked'))
	{
		flag = 1;
		$("#txt_selectallclg").empty();
		$("#txt_selectallclg").append('Unselect All Colleges');
	}
	else
	{
		flag = 0;
		$("#txt_selectallclg").empty();
		$("#txt_selectallclg").append('Select All Colleges');
	}
	var sqlkala = $("#sqlkala").val();
	var selectall_clg_count = parseInt($("#selectall_clg_count").val());
	
	if(selectall_clg_count > 250)
	{
		$('#chk_selectallclg').prop('checked', false); 
		$("#txt_selectallclg").empty();
		$("#txt_selectallclg").append('Select All Colleges');

		
		$('#selectallcollegemodal').modal('show');
	}
	else
	{
		$.ajax({
			type: "POST",
			url: "includes/set_unset_all_clg.php",
			dataType: 'text',
			data: {
				flag: flag,
				sqlkala: sqlkala
			...
			success: function (data) {
				...
				while($row = $result->fetch_assoc()) {
					...
					$unitid_element = "#clgchk".$row['UNITID'];
					
					?>
					var element = '<?php echo $unitid_element; ?>';
					if(flag == 0)
					{
						if($(element).length){
...
*/
	...
	if(isset($_SESSION['dataform1']['feestype']))
	{
		echo '$("#'.$_SESSION['dataform1']['feestype'].'").attr("checked", "checked");;';
	}
	if(isset($_SESSION['dataform1']['tuitionMin']))
	{
...
<?php  include 'footer-college.php'; ?>
