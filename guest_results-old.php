	...
	<div class="container-fluid">
		<div class="row">
			...
			<div id="progress-bar">
					...
					<div id="4"class="done chances progress-bar-item progress-bar-item-edges" ><a href="guest_results.php" class="white-font"><p class="progress-bar-text">My Chances</p></a></div>
...
if(isset($_SESSION['data_profile']) ){
	...
	*/
	...
	// if student score is > 105 then set it to 105
	if($admiscore > 105)
		$admiscore = 105;
	...
	if(strlen($selectedclgids)>0){
		...
		while ($rowclgth = mysqli_fetch_assoc($resclgth)){
			...
			$sqlSlug1 = "SELECT * FROM `hd2019` WHERE `UNITID` = '".$unitid1."'";
			...
			<?php
				...
				if(count($missedclgarr) > 0){
					...
					while ($rowclg = mysqli_fetch_assoc($resclg)){
						...
						$sqlSlug = "SELECT * FROM `hd2019` WHERE `UNITID` = '".$unitid."'";
					...
					<div class="find-out-wrap">
						<div class="estimatedbackbtn">
							<div class="blueBtnBigdiv text-center">
								<a class="btn btn-default prev mybluebtn blue-outline btn-center-xs" href="guest_college-search.php">Back</a>
							</div>
						</div>
						<div class="find-btn">
						<form action="guest_resultlistlogic.php" method="post" accept-charset="utf-8">
