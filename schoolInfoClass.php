...
class schoolInfoClass{
	...
	public function  gethd2018ById($unitid){		
		$sql = "SELECT * FROM `hd2019` WHERE `UNITID` = '".$unitid."'";
	...
	public function getType($value){		
		$sql = "SELECT `valueLabel` FROM `valuesets19` WHERE `varName` LIKE 'SECTOR' AND `tableName` = 'HD2019' AND `Codevalue` = '".$value."' ";
		$result = $this->conn->query($sql);
	...
	public function getAwardsOffered($unitid){
		$sql = "SELECT `LEVEL1`,`LEVEL2`,`LEVEL3`,`LEVEL4`,`LEVEL5`,`LEVEL6`,`LEVEL7`,`LEVEL8`,`LEVEL12`,`LEVEL17`,`LEVEL18`,`LEVEL19` FROM `ic2019` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);
		$row = $result->fetch_assoc();
		$levels = array();
		...
		if(!empty($levels)):
			...
			$qry = "SELECT `varTitle` FROM `valuesets19` WHERE `TableName` = 'ic2019' AND Codevalue =1 AND (".$awards.")
			ORDER BY `valuesets19`.`varName` ASC ";
	...
	public function getCampusSetting($codvalue){
		$sql = "SELECT `valueLabel` FROM `valuesets19` WHERE `TableName` = 'HD2019' AND `varName`='LOCALE' AND `Codevalue` = '".$codvalue."'";
		$result = $this->conn->query($sql);
		$row = $result->fetch_assoc();
		return $row;		
	}
	...
	public function getCampusHousing($unitid){
		$sql = "SELECT ROOM FROM `ic2019` WHERE `UNITID` = '".$unitid."' and ROOM = 1";
		$result = $this->conn->query($sql);
		if($result->num_rows > 0){
			$data = 'Yes';
		}else{
	...
	public function getStudentpopulation($unitid){
		$sql = "SELECT EFTOTLT FROM `ef2019a` WHERE `UNITID` = '".$unitid."' AND EFALEVEL = 1";
		$result = $this->conn->query($sql);
		$row = $result->fetch_assoc();
		return $row;	
	}
	...
	public function getFacultyRatio($unitid){		
		$sql = "SELECT STUFACR  FROM `ef2019d` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);
		$row = $result->fetch_assoc();
		return $row;
	}
	...
	public function getMissionStatement($unitid){
		$sql = "SELECT * FROM `ic2019mission` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);
		$row = $result->fetch_assoc();		
		return $row;		
	}
	...
	public function getSplLernOpport($unitid){
		$sql = "SELECT SLO5,SLO51,SLO52,SLO53,SLO6,SLO7,SLO8,SLO81,SLO82,SLO83,DSTNCED1,DSTNCED2 FROM `ic2019` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);
		$row = $result->fetch_assoc();	
		$arr = array();
		$ROTCa = array();
	...
	public function getStudentServices($unitid){
		$sql = "SELECT STUSRV1,STUSRV2,STUSRV3,STUSRV4,STUSRV8 FROM `ic2019` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);
		$row = $result->fetch_assoc();	
		$arr = array();
		if ($result->num_rows > 0) :
			foreach ($row as $key => $value) {
				if(!empty($value) && $value == 1){
					$qry = "SELECT varTitle FROM `valuesets19` WHERE `varName` LIKE '".$key."' AND Codevalue = 1 AND TableName ='IC2019'";
	...
	public function getCreditAccepted($unitid){
		$sql = "SELECT CREDITS1,CREDITS2,CREDITS3,CREDITS4 FROM `ic2019` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);
		$row = $result->fetch_assoc();	
		$arr = array();
		if ($result->num_rows > 0) :
	...
	public function getClassification($unitid){
		$sql = "SELECT C18BASIC FROM `hd2019` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);
		$row = $result->fetch_assoc();	
		$arr = array();
		foreach ($row as $key => $value) {			
			$qry = "SELECT valueLabel FROM `valuesets19` WHERE varName LIKE 'C18BASIC' AND TableName LIKE 'hd2019' AND Codevalue = '".$value."'";
			$resultt = $this->conn->query($qry);
			$rows = $resultt->fetch_assoc();	
			$arr[] = $rows['valueLabel'];	
	...
	public function getReligiousAffi($unitid){
		$sql = "SELECT RELAFFIL FROM `ic2019` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);
		$row = $result->fetch_assoc();	
		$arr = array();
		if ($result->num_rows > 0) :
			foreach ($row as $key => $value) {			
				$qry = "SELECT valueLabel FROM `valuesets19` WHERE varName LIKE 'RELAFFIL' AND TableName LIKE 'ic2019' AND Codevalue = '".$value."'";
				$resultt = $this->conn->query($qry);
				$rows = $resultt->fetch_assoc();	
				$arr[] = $rows['valueLabel'];				
			}
	...
	public function getDisabilityServices($unitid){	
		$sql = "SELECT DISAB,DISABPCT FROM `ic2019` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);
		$row = $result->fetch_assoc();			
		$services = "";
		if($row['DISAB'] == 1):
	...
	public function getOtherChar($unitid){
		$sql = "SELECT HBCU,TRIBAL FROM `hd2019` WHERE `UNITID` = '".$unitid."' AND (HBCU = 1 OR TRIBAL = 1)  ";
		$result = $this->conn->query($sql);
		if ($result->num_rows > 0) {
			$row = $result->fetch_assoc();
			$chr = "";
	...
	public function getFacultyAssistance($unitid){
		$sql = "SELECT * FROM `eap2019` WHERE `UNITID` = '".$unitid."' AND EAPCAT in(20000,21000) ";
		$result = $this->conn->query($sql);
		$faculty = array();	
		if ($result->num_rows > 0) {
	...
	public function getGraduateAssistants($unitid){
		$sql = "SELECT * FROM `eap2019` WHERE `UNITID` = '".$unitid."' AND OCCUPCAT in(410,420) ";
		$result = $this->conn->query($sql);
		$faculty = array();	
		$totalGraduateFull = 0;
		$totalGraduatePart = 0;
	...
	public function getStudentExp($unitid){
		...
		$sql = "SELECT CHG2AY1,CHG2AY2,CHG2AY3,CHG3AY1,CHG3AY2,CHG3AY3,CHG4AY1,CHG4AY2,CHG4AY3 FROM `ic2019_ay` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);
		$stdExp = array();
		if ($result->num_rows > 0) {
			$row = $result->fetch_assoc();
			...
			if(!empty($row['CHG2AY1'])):
				// In-state
				$stdExp['GTustionFees']['inState']['last_pre'] =$row['CHG2AY1'];
				$stdExp['GTustionFees']['inState']['previous'] =$row['CHG2AY2'];
				$stdExp['GTustionFees']['inState']['current'] =$row['CHG2AY3'];
			endif;		

			// Out-of-state	
			if(!empty($row['CHG3AY1'])):
				$stdExp['GTustionFees']['outOfState']['last_pre'] =$row['CHG3AY1'];
				$stdExp['GTustionFees']['outOfState']['previous'] =$row['CHG3AY2'];
				$stdExp['GTustionFees']['outOfState']['current'] =$row['CHG3AY3'];
			endif;				

			// Books and supplies
			if(!empty($row['CHG4AY1'])):
				$stdExp['GTustionFees']['booksSupplies']['last_pre'] =$row['CHG4AY1'];
				$stdExp['GTustionFees']['booksSupplies']['previous'] =$row['CHG4AY2'];
				$stdExp['GTustionFees']['booksSupplies']['current'] =$row['CHG4AY3'];
			endif;
	...
	public function getLivingArrangement($unitid){
		//group Tuition and fees
		$sql = "SELECT CHG5AY1,CHG5AY2,CHG5AY3,CHG6AY1,CHG6AY2,CHG6AY3,CHG7AY1,CHG7AY2,CHG7AY3,CHG8AY1,CHG8AY2,CHG8AY3,CHG9AY1,CHG9AY2,CHG9AY3 FROM `ic2019_ay` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);
		$stdExp = array();
		if ($result->num_rows > 0) {
			$row = $result->fetch_assoc();
			...
			if(!empty($row['CHG5AY1'])):
				...
				$stdExp['onCampus']['roomBoard']['last_pre'] =$row['CHG5AY1'];
				$stdExp['onCampus']['roomBoard']['previous'] =$row['CHG5AY2'];
				$stdExp['onCampus']['roomBoard']['current'] =$row['CHG5AY3'];
			endif;		

			// On Campus - Other
			if(!empty($row['CHG6AY1'])):
				$stdExp['onCampus']['other']['last_pre'] =$row['CHG6AY1'];
				$stdExp['onCampus']['other']['previous'] =$row['CHG6AY2'];
				$stdExp['onCampus']['other']['current'] =$row['CHG6AY3'];
			endif;

			// Off Campus - Room and board
			if(!empty($row['CHG7AY1'])):
				$stdExp['offCampus']['roomBoard']['last_pre'] =$row['CHG7AY1'];
				$stdExp['offCampus']['roomBoard']['previous'] =$row['CHG7AY2'];
				$stdExp['offCampus']['roomBoard']['current'] =$row['CHG7AY3'];
			endif;		

			// Off Campus - Other
			if(!empty($row['CHG8AY1'])):
				$stdExp['offCampus']['other']['last_pre'] =$row['CHG8AY1'];
				$stdExp['offCampus']['other']['previous'] =$row['CHG8AY2'];
				$stdExp['offCampus']['other']['current'] =$row['CHG8AY3'];
			endif;

			// Off Campus with Family
			if(!empty($row['CHG9AY1'])):
				$stdExp['offCampusFamily']['other']['last_pre'] =$row['CHG9AY1'];
				$stdExp['offCampusFamily']['other']['previous'] =$row['CHG9AY2'];
				$stdExp['offCampusFamily']['other']['current'] =$row['CHG9AY3'];
			endif;
	...
	public function getAvgGrdFees($unitid){
		$sql = "SELECT TUITION5,FEE1,TUITION7,FEE7 FROM `ic2019_ay` WHERE `UNITID` = '".$unitid."'  AND TUITION5 <> ''";
		$result = $this->conn->query($sql);
		$stdExp = array();
		if ($result->num_rows > 0) {
			$row = $result->fetch_assoc();			
	...
	public function getAltTutionPlan($unitid){
		$sql = "SELECT TUITPL1,TUITPL2,TUITPL3,TUITPL4 FROM `ic2019` WHERE `UNITID` = '".$unitid."'  AND TUITPL = 1";
		$result = $this->conn->query($sql);		
		if ($result->num_rows > 0) {
			$row = $result->fetch_assoc();					
			return $row;		
	...
	public function getFinancialAid($unitid){
		$sql = "SELECT * FROM `sfa1819_p1` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);		
		if ($result->num_rows > 0) {
			$row = $result->fetch_assoc();				
			return $row;
	...
	public function getNetPrice($unitid){
		$sql = "SELECT `NPIST0`,`NPIST1`,`NPIST2`,`NPGRN0`,`NPGRN1`,`NPGRN2` FROM `sfa1819_p2` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);		
		if ($result->num_rows > 0) {
			$row = $result->fetch_assoc();
			return $row;
	...
	public function getNetPriceTitleIV($unitid){
		$sql = "SELECT `NPIS410`,`NPT410`,`NPIS420`,`NPIS430`,`NPIS440`,`NPIS450`,`NPIS411`,`NPIS421`,`NPIS431`,`NPIS441`,`NPIS451`,`NPIS412`,`NPIS422`,`NPIS432`,`NPIS442`,`NPIS452`,`NPT411`,`NPT412`,`NPT420`,`NPT421`,`NPT422`,`NPT430`,`NPT431`,`NPT432`,`NPT440`,`NPT441`,`NPT442`,`NPT450`,`NPT451`,`NPT452` FROM `sfa1819_p2` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);		
		if ($result->num_rows > 0) {			
			$row = $result->fetch_assoc();			
			return $row;
	...
	public function getEnrollment($unitid){
		$sql = "SELECT `EFUG`,`EFGRAD`,`EFUGTRN` FROM `drvef2019` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);		
		if ($result->num_rows > 0) {			
			$row = $result->fetch_assoc();					
			return $row;
	...
	public function getUnderRaceEthnicity($unitid){
		$sql = "SELECT `PCUENRAN`,`PCUENRAP`,`PCUENRBK`,`PCUENRHS`,`PCUENRWH`,`PCUENR2M`,`PCUENRUN`,`PCUENRNR` FROM `drvef2019` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);		
		if ($result->num_rows > 0) {			
			$row = $result->fetch_assoc();	
			$dataaa = array();
	...
	public function getUndergraduateAge($unitid){
		$sql = "SELECT `DVEF13`,`DVEF14`,`DVEF15`,`DVEF16` FROM `drvef2019` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);		
		if ($result->num_rows > 0) {			
			$row = $result->fetch_assoc();					
			$dataaa = array();
	...
	public function getundergraduateResidence($unitid){
		$sql = "SELECT `RMINSTTP`,`RMOUSTTP`,`RMFRGNCP`,`RMUNKNWP` FROM `drvef2019` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);		
		if ($result->num_rows > 0) {			
			$row = $result->fetch_assoc();					
			$dataaa = array();
	...
	public function getGrdPerDistEdu($unitid){
		$sql = "SELECT `PCGDEEXC`,`PCGDESOM`,`PCGDENON` FROM `drvef2019` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);		
		if ($result->num_rows > 0) {			
			$row = $result->fetch_assoc();					
			$dataaa = array();
	...
	public function getUnderPerDistEdu($unitid){
		$sql = "SELECT `PCUDEEXC`,`PCUDESOM`,`PCUDENON` FROM `drvef2019` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);		
		if ($result->num_rows > 0) {			
			$row = $result->fetch_assoc();					
			$dataaa = array();
	...
	public function getGradStatus($unitid){
		$sql = "SELECT `EFGRAD`,`EFGRADFT`,`EFGRADPT` FROM `drvef2019` WHERE `UNITID` = '".$unitid."'";
		$result = $this->conn->query($sql);	
		$dataaa = array();		
		if ($result->num_rows > 0) {			
			$row = $result->fetch_assoc();	
	...
	public function getUndergraduateGender($unitid){
		$sql = "SELECT `EFTOTLT`,`EFTOTLM`,`EFTOTLW` FROM `ef2019a` WHERE `UNITID` = '".$unitid."' AND EFALEVEL=2 ";
		$result = $this->conn->query($sql);	
		$dataaa = array();		
		if ($result->num_rows > 0) {			
			$row = $result->fetch_assoc();			
	...
	public function getUndergraduateAttendence($unitid){		
		$sql = "SELECT `EFAGE05`,`EFAGE09`,`EFAGE06` FROM `ef2019b` WHERE `UNITID` = '".$unitid."' and LSTUDY = 2 AND EFBAGE = 1 ";
		$result = $this->conn->query($sql);	
		$dataaa = array();		
		if ($result->num_rows > 0) {			
			$row = $result->fetch_assoc();			
	...
	public function getAdmissionFall($unitid){
		$sql = "SELECT adm.APPLCN,adm.APPLCNM,adm.APPLCNW,adm.ENRLT,adm.ENRLM,adm.ENRLW,rvadm.DVADM01 ,rvadm.DVADM02,rvadm.DVADM03,adm.ADMSSN FROM `adm2019` adm LEFT JOIN drvadm2019 rvadm ON adm.UNITID = rvadm.UNITID WHERE adm.UNITID = '".$unitid."'";
		$result = $this->conn->query($sql);	
		$dataaa = array();		
		if ($result->num_rows > 0) {			
			$row = $result->fetch_assoc();
	...
	public function getAdmConsideration($unitid){
		$sql = "SELECT `ADMCON1`,`ADMCON2`,`ADMCON3`,`ADMCON4`,`ADMCON5`,`ADMCON6`,`ADMCON7`,`ADMCON8`,`ADMCON9`,`SATNUM`,`SATPCT`,`ACTNUM`,`ACTPCT`,`SATVR25`,`SATVR75`,`SATMT25`,`SATMT75`,`ACTCM25`,`ACTCM75`,`ACTEN25`,`ACTEN75`,`ACTMT25`,`ACTMT75` FROM `adm2019` WHERE `UNITID` = '".$unitid."' ";
		$result = $this->conn->query($sql);	
		$dataaa = array();
		if ($result->num_rows > 0) {			
			$row = $result->fetch_assoc();
	...
	public function getRetentionRate($unitid){
		$sql = "SELECT `RET_PCF`,`RET_PCP` FROM `ef2019d` WHERE `UNITID` = '".$unitid."' ";
		$result = $this->conn->query($sql);	
		$dataaa = array();
		if ($result->num_rows > 0) {			
			$row = $result->fetch_assoc();	
	...
	public function getOverallRate($unitid){
		$sql = "SELECT `PGGRRTT`,`TRRTTOT` FROM `drvgr2019` WHERE `UNITID` = '".$unitid."' ";
		$result = $this->conn->query($sql);	
		$dataaa = array();
		if ($result->num_rows > 0) {			
			$row = $result->fetch_assoc();	
	...
	public function getBachelorGrdRate($unitid){
		$sql = "SELECT `drvgr2019`.`GBA4RTT`,`drvgr2019`.`GBA6RTT`,`gr200_19`.`BAGR100`,`gr200_19`.`BAGR150`,`gr200_19`.`BAGR200` FROM `drvgr2019` 
		join `gr200_19` on `drvgr2019`.`UNITID`=`gr200_19`.`UNITID`
		WHERE  `drvgr2019`.`UNITID` = '".$unitid."' ";
		$result = $this->conn->query($sql);	
		...
		if ($result->num_rows > 0) {
			...
			if(!empty($row['GBA4RTT']) || !empty($row['GBA6RTT'])):
				...
				$dataset[0]['seriesname'] = "Began in Fall 2013";
				$dataset[0]['data'] = $data;
				
				$dataset[1]['seriesname'] = "Began in Fall 2011";
	...
	public function getBachelorGrdRateRace($unitid){
		$sql = "SELECT `GBA6RTAN`,`GBA6RTAS`,`GBA6RTBK`,`GBA6RTHS`,`GBA6RTNH`,`GBA6RTWH`,`GBA6RTUN`,`GBA6RTNR`,`GBA6RTM`,`GBA6RTW` FROM `drvgr2019` WHERE `UNITID` = '".$unitid."' ";
		$result = $this->conn->query($sql);	
		$dataaa = array();
		$datgender =  array();
		if ($result->num_rows > 0) {			
	...
	public function getOutcomeMeasff($unitid){
		$sql = "SELECT `OM1TotlBACp8`,`OM1TotlEnYp8`,`OM1TotlEnAp8`,`OM1PellBACp8`,`OM1PellEnYp8`,`OM1PellEnAp8`,`OM1nPelBACp8`,`OM1nPelEnYp8`,`OM1nPelEnAp8` FROM `drvom2019` WHERE `UNITID` = '".$unitid."' ";
		$result = $this->conn->query($sql);	
...
public function getOutcomeMeaspf($unitid){
		$sql = "SELECT `OM2TotlBACp8`,`OM2PellBACp8`,`OM2nPelBACp8`,`OM2TotlEnYp8`,`OM2PellEnYp8`,`OM2nPelEnYp8`,`OM2TotlEnAp8`,`OM2PellEnAp8`,`OM2nPelEnAp8` FROM `drvom2019` WHERE `UNITID` = '".$unitid."' ";
		$result = $this->conn->query($sql);	
	...
	public function getOutcomeMeasfnf($unitid){
		$sql = "SELECT `OM3TotlBACp8`,`OM3PellBACp8`,`OM3nPelBACp8`,`OM3TotlEnYp8`,`OM3PellEnYp8`,`OM3nPelEnYp8`,`OM3TotlEnAp8`,`OM3PellEnAp8`,`OM3nPelEnAp8` FROM `drvom2019` WHERE `UNITID` = '".$unitid."' ";
		$result = $this->conn->query($sql);	
	...
	public function getOutcomeMeaspnf($unitid){
		$sql = "SELECT `OM4TotlBACp8`,`OM4PellBACp8`,`OM4nPelBACp8`,`OM4TotlEnYp8`,`OM4PellEnYp8`,`OM4nPelEnYp8`,`OM4TotlEnAp8`,`OM4PellEnAp8`,`OM4nPelEnAp8` FROM `drvom2019` WHERE `UNITID` = '".$unitid."' ";
		$result = $this->conn->query($sql);	
	...
	public function getServicemembers($unitid){
		$sql = "SELECT `UGPO9_N`,`GPO9_N`,`UGDOD_N`,`GDOD_N` FROM `sfav1819` WHERE `UNITID` = '".$unitid."' ";
		$result = $this->conn->query($sql);	
		$dataaa = array();
		$datgender =  array();
		if ($result->num_rows > 0) {			
	...
	public function getServicemembersA($unitid){
		$sql = "SELECT `UGPO9_A`,`GPO9_A`,`UGDOD_A`,`GDOD_A` FROM `sfav1819` WHERE `UNITID` = '".$unitid."' ";
		$result = $this->conn->query($sql);	
		$dataaa = array();
		$datgender =  array();
		if ($result->num_rows > 0) {			
	...
	public function getProgramMajor($unitid){
		...
		$sql = "SELECT * FROM `c2019dep` INNER JOIN valuesets19 ON c2019dep.CIPCODE=valuesets19.Codevalue WHERE `c2019dep`.`UNITID` = '".$unitid."' AND `valuesets19`.`TableName` LIKE 'C2019DEP' AND `valuesets19`.`varName` LIKE 'CIPCODE' GROUP BY FLOOR(`c2019dep`.`CIPCODE`) ORDER BY `valuesets19`.`valueLabel` ASC";
		
		$result = $this->conn->query($sql);	
		$dataa = "";
		if ($result->num_rows > 0) {
			$dataa .= "
			<p class='note'><b>COMPLETIONS (NUMBER OF AWARDS CONFERRED) 2018-2019</b> <br />
			...
			while($row = $result->fetch_assoc()) {
				...
				$qry ="SELECT `valueLabel` FROM `valuesets19` WHERE `TableName` LIKE 'C2019DEP' AND `varName` LIKE 'CIPCODE' AND `Codevalue` LIKE '".$CIPCODE."' ";
				...
				$qry1 ="SELECT * FROM `c2019dep` WHERE `UNITID` = '".$unitid."' AND CIPCODE LIKE '".$CIPCODE."%'";
...
$dataa .= "</tbody></table></div>";	
	...
	public function getProgramMajorData($unitid,$CIPCODE,$AWLEVEL,$disc ){		
		$sql = "SELECT `CIPCODE`,`AWLEVEL`,`CTOTALT` FROM `c2019_a` WHERE `UNITID` = '".$unitid."' and CIPCODE = '".$CIPCODE."' AND AWLEVEL = '".$AWLEVEL."' ";
		$result = $this->conn->query($sql);	
		$total = "-";
		$des = "";
		if ($result->num_rows > 0) {	
	...
	public function labelHeading($CIPCODE){
		$qry ="SELECT `valueLabel` FROM `valuesets19` WHERE `TableName` LIKE 'C2018DEP' AND `varName` LIKE 'CIPCODE' AND `Codevalue` LIKE '".$CIPCODE."' ";
		$res = $this->conn->query($qry);
		if ($res->num_rows > 0) {
			$rowRes = $res->fetch_assoc();
	...
	public function shortInfoStudent($unitid){
		$sql = "SELECT `EFAGE09`,`EFAGE01`,`EFAGE02`,`EFAGE05`,`EFAGE06` FROM `ef2019b` WHERE `UNITID` = '".$unitid."' ANd LSTUDY = 1 AND EFBAGE = 1 ";
	...
	public function getUnderDegNon($unitid){
		$sql = "SELECT EFTOTLT FROM `ef2019a` WHERE `UNITID` = '".$unitid."' AND EFALEVEL = 31";
		$result = $this->conn->query($sql);
		$row = $result->fetch_assoc();
		return $row;
	}

	public function newsApi($clgName,$newsFeed){		
		// Fist check for News Feed
		$newsData = false;
		$res = array();
		if(!empty($newsFeed)){
			$feeds = simplexml_load_file($newsFeed);
			if(!empty($feeds)){
				$newsData = true;
				$res['newsFeed'] = $feeds;
				return $res;
		...
		if($newsData == false){
			...
			$query = rawurlencode($clgName);
			$source = 'techcrunch.com';
			$ch = curl_init();

			curl_setopt($ch, CURLOPT_URL, 'api.datanews.io/v1/news?q=%22'.$query.'%22&sortBy=date&language=en');
			...
			if(!empty($res12->hits)):	
				...
				foreach ($info as $key => $value) {
					...
					if (stripos($title, $clgName) !== false || stripos($description, $clgName) !== false || stripos($content, $clgName) !== false) {
